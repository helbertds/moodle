...
class functions {
    ...
    public static function sigfig($number, $precision): string {
        ...
        // For very small or very large numbers, use scientific notation to avoid
        // unreadable strings of zeros. We switch to scientific notation when the
        // required decimal places exceed precision + 2 (small numbers like 2.838E-19)
        // or when the number has more than 15 digits before the decimal point
        // (large numbers approaching float precision limits).
        if ($number != 0 && ($digitsafter > $precision + 2 || $digitsbefore > 15)) {
            $sign = $number < 0 ? '-' : '';
            $exp = floor(log10(abs($number)));
            $mantissa = abs($number) / pow(10, $exp);
            $mantissa = round($mantissa, $precision - 1);
            // If rounding the mantissa pushed it to 10, adjust.
            if ($mantissa >= 10.0) {
                $mantissa /= 10.0;
                $exp += 1;
            }
            $decimalplaces = max(0, $precision - 1);
            return $sign . number_format($mantissa, $decimalplaces, '.', '') . 'E' . intval($exp);
        }
